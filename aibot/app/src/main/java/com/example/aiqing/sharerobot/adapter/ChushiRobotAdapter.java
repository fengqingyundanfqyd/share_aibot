...
public class ChushiRobotAdapter extends BaseAdapter {
    private final Context context;
    private final List<KucunBean.ProListBean.ObjBean.ResultBean> result;

    public ChushiRobotAdapter(Context context, List<KucunBean.ProListBean.ObjBean.ResultBean> result) {
...
//        viewHolder.tvDate.setText("2017-7-3");//服务器返回空
        ...
        if (doStatus.equals("1")){
            ...
            viewHolder.tv_agent_status.setText("已审核");
        }else if (doStatus.equals("4")){
            viewHolder.tv_agent_status.setText("已拒绝");
        }else if (doStatus.equals("5")){
            viewHolder.tv_agent_status.setText("待处理");
        }else if (doStatus.equals("6")){
            viewHolder.tv_agent_status.setText("已处理");
        }else if (doStatus.equals("7")){
            viewHolder.tv_agent_status.setText("已拒绝");
        }else if (doStatus.equals("8")){
            viewHolder.tv_agent_status.setText("已确认");
        }else if (doStatus.equals("9")){
            viewHolder.tv_agent_status.setText("已取消");
